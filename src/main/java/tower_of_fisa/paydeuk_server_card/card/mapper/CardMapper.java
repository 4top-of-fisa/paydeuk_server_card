...
public class CardMapper {
  ...
  private List<CardBenefitResponse> mapCardBenefits(List<CardBenefit> cardBenefits) {
    return cardBenefits.stream().map(this::mapCardBenefit).toList();
  }
  ...
  private List<MerchantResponse> mapMerchants(
      tower_of_fisa.paydeuk_server_card.domain.entity.Merchant merchant) {
    if (merchant == null) {
      return List.of();
    }
    return List.of(MerchantResponse.builder().merchantName(merchant.getName()).build());
  }

  private List<BenefitConditionResponse> mapBenefitConditions(
      List<BenefitCondition> benefitConditions) {
    return benefitConditions.stream().map(this::mapBenefitCondition).toList();
  }
  ...
  private List<DiscountRateResponse> mapDiscountRates(List<DiscountRate> discountRates) {
    return discountRates.stream().map(this::mapDiscountRate).toList();
  }
