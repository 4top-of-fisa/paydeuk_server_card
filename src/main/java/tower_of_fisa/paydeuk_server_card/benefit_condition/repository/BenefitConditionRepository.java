...
public interface BenefitConditionRepository extends JpaRepository<BenefitCondition, Long> {
  ...
  @Query(
      "SELECT bc FROM BenefitCondition bc "
          + "LEFT JOIN bc.spendingRange sr "
          + "WHERE bc.benefit = :benefit "
          + "AND ( "
          + "(sr.minSpending <= :previousMonthSpending AND sr.maxSpending > :previousMonthSpending) "
          + "OR (sr.minSpending <= :previousMonthSpending AND sr.maxSpending IS NULL) "
          + "OR (bc.spendingRange IS NULL)"
          + ")")
